...
from veduta_data.library_writer import atomic_write, load_catalog, load_collection, selected_collections, update_wallpaper_metadata
...
def _copy_atomic(source: Path, destination: Path) -> None:
    with source.open("rb") as source_file:
        atomic_write(destination, lambda temp_file: shutil.copyfileobj(source_file, temp_file, length=1024 * 1024))
...
def import_installed_pack_images(
    *,
    ...
    for collection_summary in selected_collections(catalog, collection_id, collection_id is None):
        manifest = str(collection_summary["manifest"])
        manifest_path = library_root / manifest
        collection = load_collection(library_root, manifest)
